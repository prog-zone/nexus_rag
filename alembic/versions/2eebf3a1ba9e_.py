...
Revision ID: 2eebf3a1ba9e
Revises: 
Create Date: 2026-05-02 03:56:04.230520
...
def upgrade() -> None:
    ...
    sa.Column('status', sa.Enum('ACTIVE', 'ARCHIVED', 'CLOSED', name='casestatus'), nullable=False),
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
    ...
    op.create_index(op.f('ix_chats_case_id'), 'chats', ['case_id'], unique=False)
    ...
    sa.Column('is_in_qdrant', sa.Boolean(), nullable=False),
    ...
    sa.Column('source', sa.Enum('UPLOAD', 'INLINE_PASTE', name='documentsource'), nullable=False),
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_chats_case_id'), table_name='chats')
    op.drop_table('chats')
